...
ACTIVITY_QUERIES = Depends(ActivityQueries)
...
async def list_activities(
    user_id: uuid.UUID,
    activity_queries: ActivityQueries = ACTIVITY_QUERIES,
) -> List[Activity]:
...
@router.get("/activities/{activity_id}/details", response_model=Activity)
...
@router.post("/activities/create", response_model=Activity)
async def create_new_activity(
    ...
    """
    activity_params = await dummy_call_remote_activity_generation_endpoint(user_id)
    async with db_client.pool.connection() as conn:
        created_activity = await create_activity(conn, activity_params)
    return created_activity


@router.post("/activities/{activity_id}/start", response_model=ActivityItem)
...
@router.post("/activities/{activity_id}/items/{activity_item_id}/answer")
